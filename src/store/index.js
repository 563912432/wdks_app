...
const store = new Vuex.Store({
  state: {
    ...
    extraJudgeCourse: [21, 22, 23, 24, 25], // 初中级会计 判分规则特殊
    isLogin: false,
    selected: '1',
    userInfo: null,
    isWxBrowser: false,
  ...
  mutations: {
    setCurWenkuCate (state, data) {
      state.cur_wenku_cate = data
    },
    logout (state) {
      state.isLogin = false
      state.userInfo = null
    },
    setAdShowSwitch (state, data) {
      state.adShowSwitch = data
    },
    ...
    setExamNote (state, bool) {
      state.examState.examNote = bool
    },
    ...
    clearExamState (state) {
      ...
      state.examState.examNote = false
    },
  ...
  actions: {
    ...
    getPaperList ({commit, state}, data) {
      Vue.http.get(state.host + 'Api/Paper/getPaperList/type/' + data.param.type + '/cid/' + data.param.cid + '/uid/' + data.param.uid, {timeout: 5000}).then(response => {
        if (response.ok && response.body.status === 1) {
          data.success(JSON.parse(response.body.info))
        } else {
          data.error(response.body.info)
    ...
    // 提交笔记
    submitNote ({commit, state}, data) {
      Vue.http.post(state.host + 'Api/Note/submitNote', data.params, {
        timeout: 5000,
        emulateJSON: true
      }).then(response => {
        data.callback(response.data)
    ...
    getArticleList ({commit, state}, data) {
      Vue.http.get(state.host + 'Api/News/getArticleList' + (data.cid ? '/cid/' + data.cid : '') + '/p/' + data.p, {timeout: 5000}).then(response => {
        if (response.ok && response.body.status === 1) {
          data.success(JSON.parse(response.body.info))
        } else if (response.body.status === -1) {
          commit('logout')
    ...
    // 获取备考文库列表
    getNoteList ({commit, state}, data) {
      Vue.http.get(state.host + 'Api/Note/getList/uid/' + data.uid + (data.cid ? '/cid/' + data.cid : '') + (data.searchOption ? '/searchOption/1' : '') + (data.noteDate ? '/noteDate/' + data.noteDate : '') + '/p/' + data.p, {timeout: 5000}).then(response => {
        if (response.ok && response.body.status === 1) {
          data.success(JSON.parse(response.body.info))
        } else if (response.body.status === -1) {
          commit('logout')
