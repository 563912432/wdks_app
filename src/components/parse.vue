<template>
  <div class="parse">
    <div class="waiter-mask" v-if="waiterExist" v-show="waiterShow" @click="closeWaiter"></div>
    <div class="waiter-button" v-if="waiterExist" @click="showWaiter"><i class="icon iconfont icon-kujialeqiyezhan_zaixiankefu-" style="font-size: 50px;"></i></div>
    <div class="waiter-wrap" v-if="waiterExist" v-show="waiterShow" @click="closeWaiter">
      <img :src="uploadPath + waiterWeixin" />
      <div style="text-align: center; padding: 10px;">关注老师微信，免费领取资料</div>
    </div>
    <div class="head">
    ...
    <div class="list" v-if="allExam">
      <div class="list-for" v-for="(exam, ind) in allExam">
        <div :class="ind % 2 === 0 ? odd : even" v-if="+exam.type === typeList.single ||
          ...
          <div class="row">
            <mt-button size="small" @click="notes(exam.number)"><i class="icon iconfont icon-baoming"></i>笔记</mt-button>
          </div>
        </div>
        <!--判断题-->
...
<script type="text/ecmascript-6">
  ...
  export default {
    mixins: [COMMON],
    components: {
      Toast, parseVideo, note
    },
    computed: {
      ...mapState(['typeList']),
      ...mapGetters(['uploadPath']),
      waiterWeixin () {
        return this.$store.state.userInfo.waiter_weixin
      },
      waiterExist () {
        return (this.$store.state.userInfo.waiter_weixin && this.$store.state.userInfo.waiter_weixin !== '')
      }
    },
    data () {
      return {
    ...
    methods: {
      notes (number) {
        ...
        this.$store.commit('setExamNote', true)
      },
      showWaiter () {
          ...
          for (let n in this.userAnswer) {
            if (typeof number === 'string') {
              if (number === this.userAnswer[n].no) {
                console.dir(this.userAnswer[n].val)
                return ((typeof this.userAnswer[n].val === 'object') ? this.userAnswer[n].val.slice(0).sort().toString() : this.userAnswer[n].val.toString())
              }
            } else if (typeof number === 'object') {
              if (number[0] === this.userAnswer[n].no) {
                for (let i = 0; i < this.userAnswer[n].val.length; i++) {
                  if (number[1] === this.userAnswer[n].val[i].key) {
                    return ((typeof this.userAnswer[n].val[i].subval === 'object') ? this.userAnswer[n].val[i].subval.slice(0).sort().toString() : this.userAnswer[n].val[i].subval.toString())
                  }
                }
              }
            }
