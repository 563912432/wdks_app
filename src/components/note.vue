  ...
  <div>
    <mt-popup v-model="isShow" position="bottom" :closeOnClickModal="false">
      ...
      <div class="parse-info">
        <h2>做题笔记</h2>
        <div class="info">
          <mt-field class="textarea" @input='desc' @change="setNote()" placeholder="我的笔记" type="textarea" rows="7" :attr="{ maxlength: 250 }" v-model="note">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    props: ['number', 'defaultNote'],
    computed: {
      ...mapGetters(['createSignature']),
      isShow: {
        get: function () {
          return this.$store.state.examState.examNote
        },
        set: function (newValue) {
          this.$store.state.examState.examNote = newValue
        }
      }
    },
    watch: {
      isShow () {
        this.note = this.$store.state.noteContent
      }
    },
    methods: {
      setVisible () {
        this.$store.commit('setExamNote', false)
        this.note = ''
      },
      desc () {
        ...
        if (this.note === '') {
          ...
          let data = {
            params: {
              ...
              course_id: (this.$route.params.tid ? this.$route.params.tid : this.$store.state.paperInfo.course_id),
              note: this.$store.state.noteContent
            },
          ...
          this.$store.dispatch('submitNote', data)
          this.note = ''
        }
      }
    }
