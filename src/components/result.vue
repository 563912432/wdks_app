    ...
    beforeRouteEnter (to, from, next) {
      if (from.name === 'exam') {
        next(vm => {
          ...
          let errorNumber = []
          let panduanWrongNum = 0
          let panduanRightNum = 0
          ...
          // 初中级会计判断题不定项选择题判分特殊
          if (vm.$store.state.extraJudgeCourse.indexOf(+vm.$store.state.paperInfo.course_id) !== -1) {
            // 判断题做错倒扣
            let panduanScore = panduanRightNum * paperInfo['5'].score
            let panduanReduce = panduanWrongNum * 0.5
            let panduanExtra = (panduanScore > panduanReduce ? panduanReduce : panduanScore)
            if (panduanExtra > 0) {
              vm.score = vm.score - panduanExtra
            }
            // 不定项选择题
          }
