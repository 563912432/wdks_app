    ...
    <div class="list" v-if="list">
      <router-link class="item" v-for="item in list" :key="item.index"
        ...
        <div class="p_title">
          ...
          <div style="font-size:10px;padding-left: 30px;color:#999;">进度 ( {{item.done_num}} / {{item.tk_num}} )</div>
        </div>
      </router-link>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getPaperList () {
        let that = this
        let data = {
          param: {type: this.$store.state.special, cid: this.$route.params.tid, uid: this.$store.state.userInfo.id},
